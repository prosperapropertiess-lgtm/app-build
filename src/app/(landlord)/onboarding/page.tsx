    ...
    <div className="min-h-screen bg-zinc-950 flex items-center justify-center p-4">
      <div className="max-w-lg w-full">
          ...
          {STEPS.map((s) => (
            <div
              ...
              className={`w-3 h-3 rounded-full transition-colors ${
                s.id <= step ? "bg-green-500" : "bg-zinc-800"
              }`}
            />
          ))}
        </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 space-y-5">
          {step === 1 && (
            <div className="space-y-4 text-center">
              <p className="text-zinc-400">Welcome to Project X! Let's get you set up so you can start managing your properties.</p>
              <p className="text-zinc-400">This quick setup will have you ready in under 2 minutes.</p>
            </div>
          )}

          {step === 2 && (
            <div className="space-y-4">
              <p className="text-zinc-400">Let's start with your first property.</p>
              <input
                type="text"
          ...
          {step === 3 && (
            <div className="space-y-4">
              <p className="text-zinc-400">Invite a tenant to get started. They&apos;ll receive an email with login instructions.</p>
              <input
                type="email"
              ...
              />
              <p className="text-zinc-500 text-sm">You can skip this and invite tenants later.</p>
            </div>
          )}

          {step === 4 && (
            <div className="space-y-4 text-center">
              <p className="text-zinc-400">Set up Stripe to accept rent payments online.</p>
              <div className="bg-zinc-800 rounded-xl p-6">
                <span className="text-4xl mb-4 block">💳</span>
                <p className="text-zinc-300 text-sm">Stripe Connect integration coming soon</p>
              </div>
              <p className="text-zinc-500 text-sm">Skip for now — you can enable payments anytime.</p>
            </div>
          )}

          {step === 5 && (
            <div className="space-y-4 text-center">
              <p className="text-zinc-400">You&apos;re all set! Your property management portal is ready.</p>
              <div className="grid grid-cols-3 gap-4 pt-4">
                <div className="bg-zinc-800 rounded-xl p-4 text-center">
                  <span className="text-2xl">🏠</span>
                  <p className="text-zinc-400 text-xs mt-2">Properties</p>
                </div>
                <div className="bg-zinc-800 rounded-xl p-4 text-center">
                  <span className="text-2xl">👤</span>
                  <p className="text-zinc-400 text-xs mt-2">Tenants</p>
                </div>
                <div className="bg-zinc-800 rounded-xl p-4 text-center">
                  <span className="text-2xl">💰</span>
                  <p className="text-zinc-400 text-xs mt-2">Payments</p>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="flex gap-4 mt-6">
          ...
          <button
            onClick={handleNext}
            className="flex-1 py-3 bg-green-500 hover:bg-green-400 text-zinc-950 font-semibold rounded-xl transition-colors"
          >
